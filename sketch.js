...
let showDebug = false; // Toggle the menu visibility
let moonGravity = false; // Toggle for moon gravity
let showHitboxes = false; // Additional feature: view collision boxes
...
function preload() {
  ...
  // COMMENT THESE OUT if the files are missing from your folder
...
function keyPressed() {
  ...
  if (key === "h" || key === "H") {
    showDebug = !showDebug; // Toggle debug menu visibility
  }

  if (showDebug) {
    if (key === "1") moonGravity = !moonGravity; // Toggle Moon Gravity
    if (key === "2") showHitboxes = !showHitboxes; // Toggle Hitboxes
  }
}
...
function draw() {
  ...
  fill(255);
  noStroke(); // Prevents weird outlines on text
  textSize(10);
  textAlign(LEFT, TOP);
  text("DEBUG MENU (H to Show)", 15, 15);
  text("1. Moon Gravity: " + (moonGravity ? "ON" : "OFF"), 15, 30);
  text("2. Show Hitboxes: " + (showHitboxes ? "ON" : "OFF"), 15, 45);
  text("Player Y-Vel: " + player.vel.y.toFixed(2), 15, 60);
  // --- PLAYER CONTROLS ---
  ...
  // Toggle gravity based on debug setting
  if (moonGravity) {
    world.gravity.y = 2; // Low gravity (Moon)
  } else {
    world.gravity.y = GRAVITY; // Normal gravity (Earth)
  }

  // Toggle hitbox visibility for all sprites
  allSprites.debug = showHitboxes;
  // -- JUMP --
  ...
  // --- DEBUG MENU UI ---
  if (showDebug) {
    camera.off(); // Ensure the menu stays fixed on screen
